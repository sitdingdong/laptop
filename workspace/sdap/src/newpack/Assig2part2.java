...
3 Paste the code into the java file just created
4 Change the password(pwd) and username(usr) to match the database in the executing computer
5 Run it 
...
public class Assig2part2 { 
	...
	public void signin() {
		...
		try { 
			...
			while (rs.next()) { 
				...
				// temp is to store the data of each prod 
				TreeMap<String, int[]> temp = data.get(cust);
				if(!temp.containsKey(prod)) {
					temp.put(prod, new int[8]);
				}
				// temp1 is to store the quant to calculate the avg 
	...
	public static void main(String[] args) { 
		Assig2part2 sales = new Assig2part2();
		sales.signin();
		sales.printout();
