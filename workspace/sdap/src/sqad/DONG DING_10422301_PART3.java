		...
		for(Entry<String, TreeMap<String, List<Integer>>> e : data1.entrySet()) {
			...
			for(Entry<String, List<Integer>> e1 : e.getValue().entrySet()) {
				...
				for(int i = 0; i < ls.size(); i += 2) {
					...
					if(quarter == 1) {
						if(quant < (amo[3] == 0 ? 0 : amo[3] / amo[4]) && quant > amo[5]) {
							res[2] ++;
						}
					} else if(quarter == 4) {
						if(quant < (amo[6] == 0 ? 0 : amo[6] / amo[7]) && quant > amo[8]) {
							res[5] ++; // 3 after
						}
					} else {
						if(quant < (amo[quarter*3] == 0 ? 0 : amo[quarter*3] / amo[quarter*3]) && quant > amo[quarter*3 + 2]) {
							res[quarter*2] ++; //before
						}
						if(quant < (amo[quarter*3 - 6] == 0 ? 0 : amo[quarter*3 - 6] / amo[quarter*3 - 5]) && quant > amo[quarter*3 - 4]) {
